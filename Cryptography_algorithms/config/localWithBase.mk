...
# |	|--Release
# |	|	|--libs
# |	|	|	|--base_obj -> include all objects in base folder
...
BASE_OBJ = $(patsubst base/%.cpp,$(RELEASE_DIR)/libs/base_obj/%.o,$(BASE_SRC))
...
# Compile base objects
$(RELEASE_DIR)/libs/base_obj/%.o: base/%.cpp | $(RELEASE_DIR)/libs/base_obj
	$(CXX) $(CXXFLAGS) -c $< -o $@

$(DEBUG_DIR)/libs/base_obj/%.o: base/%.cpp | $(DEBUG_DIR)/libs/base_obj
	$(CXX) $(CXXFLAGS) -c $< -o $@
...
# $(RELEASE_DIR)/libs/$(FOLDER_NAME)_export.a: $(BASE_OBJ) $(CORE_OBJ) $(EXPORT_OBJ)
# 	ar rcs $@ $^

# Create shared library
$(RELEASE_DIR)/libs/$(FOLDER_NAME)_export.so: $(BASE_OBJ) $(CORE_OBJ) $(EXPORT_OBJ)
	$(CXX) -shared -fPIC -o $@ $^
...
$(RELEASE_DIR)/libs/base_obj $(RELEASE_DIR)/libs/core_obj $(RELEASE_DIR)/libs/test_obj $(RELEASE_DIR)/libs $(RELEASE_DIR)/exe:
	mkdir -p $@

$(DEBUG_DIR)/libs/base_obj $(DEBUG_DIR)/libs/core_obj $(DEBUG_DIR)/libs/test_obj $(DEBUG_DIR)/libs $(DEBUG_DIR)/exe:
	mkdir -p $@
